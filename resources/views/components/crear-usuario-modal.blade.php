...
<div x-show="showCrearModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
    ...
    <div x-show="showCrearModal"
        ...
        <div class="bg-blue-600 text-white px-6 py-4 flex items-center shadow-sm justify-between">
            ...
            <button @click="showCrearModal = false; window.resetCrearUsuarioForm && window.resetCrearUsuarioForm()"
        ...
        <form id="crearUsuarioForm" action="{{ route('users.save') }}" method="POST">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-input type="text" name="name" placeholder="Nombre" class="w-full" x-imask="'text'"
                                required />
                        </div>
                        <div>
                            <x-label value="Apellido" class="mb-1" />
                            <x-input type="text" name="lastname" placeholder="Apellido" class="w-full" x-imask="'text'"
                                required />
                        </div>
                        ...
                        <div x-data="{ show: false }">
                            ...
                            <div class="relative">
                                <x-input ::type="show ? 'text' : 'password'" name="password" placeholder="********"
                                    class="w-full pr-10" required x-imask="{mask:/^.*$/}" />
                                <span id="crearPwdMsg" class="text-red-500 text-xs absolute top-0 right-0 mt-2 mr-10"
                                    style="display:none;">Mínimo 8 caracteres</span>
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                {{ __('La contraseña debe tener al menos 8 caracteres.') }}</p>
                        </div>
                        <div x-data="{ show: false }">
                            <x-label value="Confirmar Contraseña" class="mb-1" />
                            <div class="relative">
                                <x-input ::type="show ? 'text' : 'password'" name="password_confirmation"
                                    placeholder="********" class="w-full pr-10" required x-imask="{mask:/^.*$/}" />
                                <span id="crearConfirmMsg"
                                    class="text-red-500 text-xs absolute top-0 right-0 mt-2 mr-10"
                                    style="display:none;">Mínimo 8 caracteres</span>
                                <button type="button" @click="show = !show"
                                    class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                    <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" fill="none"
            ...
            <div
                ...
                <x-secondary-button class="px-10"
                    @click="showCrearModal = false; window.resetCrearUsuarioForm && window.resetCrearUsuarioForm()">
                    {{ __('Cancelar') }}
                </x-secondary-button>
            </div>
        </form>
