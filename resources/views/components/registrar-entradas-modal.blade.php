...
<div x-show="showModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0" 
    ...
    <div x-show="showModal" class="fixed inset-0 transform transition-all"
        @click="showModal = false; showHistorial = false" x-transition:enter="ease-out duration-300"
    ...
    <!-- Panel del Modal -->
    <div x-show="showModal" :class="showHistorial ? 'sm:max-w-7xl' : 'sm:max-w-2xl'"
        ...
        <div class="bg-blue-600 text-white px-6 py-4 flex items-center justify-between shadow-sm">
            <span class="text-xl font-bold"
                x-text="showHistorial ? '{{ __('Historial de Salidas') }}' : '{{ __('Registrar Salida de Equipo') }}'"></span>
            <div class="flex gap-2">
                <button type="button" @click="showHistorial = !showHistorial; if(showHistorial) fetchHistorial()"
                    ...
                    <span x-text="showHistorial ? '{{ __('Volver') }}' : '{{ __('Historial') }}'"></span>
                </button>
                <button @click="showModal = false; showHistorial = false"
        ...
        <div x-show="!showHistorial">
            <form action="{{ route('recepcion.salida') }}" method="POST">
                @csrf
                <div class="p-8">
        ...
        <div x-show="showHistorial" class="p-6 overflow-hidden">
            ...
            <div x-show="!loading" class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs text-left">
                    <thead class="bg-blue-600 text-white">
                        <tr>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Fecha') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Usuario Registro') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Tipo Equ.') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Serial') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Modelo') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Pieza Reemplazada') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Atención') }}</th>
                            <th class="px-3 py-3 font-bold uppercase">{{ __('Entregado por') }}</th>
                            <th class="px-3 py-3 font-bold uppercase text-center">{{ __('Acciones') }}</th>
                        </tr>
                    </thead>
                        ...
                        <template x-for="item in historial" :key="item.id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-3 py-3 whitespace-nowrap text-gray-600 dark:text-gray-300"
                                    x-text="new Date(item.created_at).toLocaleDateString()"></td>
                                <td class="px-3 py-3 text-gray-800 dark:text-gray-100 font-medium"
                                    x-text="(item.usuario_entrega?.name || '') + ' ' + (item.usuario_entrega?.lastname || '')">
                                </td>
                                <td class="px-3 py-3 text-gray-600 dark:text-gray-300"
                                    x-text="item.equipo?.tipo?.nombre || 'N/A'"></td>
                                <td class="px-3 py-3 font-mono text-blue-600 dark:text-blue-400"
                                    x-text="item.equipo?.serial_equipo || 'N/A'"></td>
                                <td class="px-3 py-3 text-gray-600 dark:text-gray-300"
                                    x-text="item.equipo?.modelo?.nombre || 'N/A'"></td>
                                <td class="px-3 py-3 text-gray-600 dark:text-gray-300 max-w-xs truncate"
                                    x-text="item.caso?.documentacion?.map(d => d.pieza_soporte?.nombre || 'Servicio').join(', ') || 'Ninguna'">
                                </td>
                                <td class="px-3 py-3 capitalize text-gray-600 dark:text-gray-300"
                                    x-text="item.caso?.forma_de_atencion || 'N/A'"></td>
                                <td class="px-3 py-3">
                                    <span
                                        :class="item.deposito === 'tecnico' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'"
                                        class="px-2 py-1 rounded-full text-[10px] font-bold uppercase"
                                        x-text="item.deposito"></span>
                                </td>
                                <td class="px-3 py-3 text-center">
                                    <a :href="'/salida/' + item.id + '/pdf'" target="_blank" class="inline-flex items-center text-red-600 hover:text-red-800 transition-colors" title="Generar PDF">
            ...
            <!-- Paginación -->
            <div x-show="!loading" class="mt-4 flex items-center justify-between text-xs font-medium">
                <span class="text-gray-500 dark:text-gray-400"
                    x-text="'Pag. ' + pagination.current_page + ' de ' + pagination.last_page"></span>
                <div class="flex gap-2">
                    <button @click="if(pagination.prev_page_url) fetchHistorial(pagination.prev_page_url)"
                        :disabled="!pagination.prev_page_url"
                        :class="!pagination.prev_page_url ? 'opacity-30 cursor-not-allowed' : 'hover:bg-blue-600 hover:text-white'"
                        class="px-4 py-1 border border-blue-600 dark:border-blue-400 text-blue-600 dark:text-blue-400 rounded transition duration-200">
                        {{ __('Anterior') }}
                    </button>
                    <button @click="if(pagination.next_page_url) fetchHistorial(pagination.next_page_url)"
                        :disabled="!pagination.next_page_url"
                        :class="!pagination.next_page_url ? 'opacity-30 cursor-not-allowed' : 'hover:bg-blue-600 hover:text-white'"
                        class="px-4 py-1 border border-blue-600 dark:border-blue-400 text-blue-600 dark:text-blue-400 rounded transition duration-200">
                        {{ __('Siguiente') }}
                    </button>
                </div>
